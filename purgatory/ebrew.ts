...
import { upsertWorkerInfo } from "../src/database/workerInfo";
import { constants } from "../src/providers/config";
import { permissions } from "../src/providers/permissions";
import { Command } from "../src/structures/Command";
import { randRange } from "../src/utils/utils";
import {
	CommandInteraction,
	ComponentType,
	EmbedBuilder,
...
export const command = new Command("ebrew", "Brews your claimed order.")
		...
		subcommand
			...
			.addStringOption((option) =>
				option
					.setName("url")
					.setDescription(
						"The URL of the image to attach to the order."
					)
					.setRequired(true)
			)
	)
	.addSubCommand((subcommand) =>
		subcommand.setName("claim").setDescription("Claim an order.")
	)
	...
	.setExecutor(async (int: CommandInteraction) => {
		...
		if (subcommand === "claim") {
			...
			if (claimedOrders.length === 0) {
				await int.reply({
					content: "You don't have any claimed orders to brew.",
				});
				return;
			}
			...
			const orderOptions = claimedOrders.map((order) => ({
				...
				description: `User: ${order.user.substring(
					0,
					15
			...
			const filter = (i) =>
				i.customId === "select_order_to_claim" &&
			...
			(subcommand === "attach" || subcommand === "url") &&
			int.client.ordersInProcess?.[int.user.id]?.orderId
		) {
			...
			if (
				...
				await int.reply({
					content:
						"You have Already Brewed this order. select an new one!",
				});
				return;
			}
			...
			if (subcommand === "attach") {
				const attachment = int.options.get(
					"attachment",
					true
				)?.attachment;
				if (!attachment) {
					await int.reply({
						content: "Attachment is missing or not valid.",
					});
					return;
				}
			...
			const confirmFilter = (i) =>
				i.customId === "confirm_brew" && i.user.id === int.user.id;
				...
				if (confirmInteraction.isStringSelectMenu()) {
					...
					if (selectedValue === "yes") {
						await confirmInteraction.update({
							content:
								"You chose to proceed with brewing the order.",
							components: [],
						});
			...
			await int.reply({
				content:
					"You need to claim an order first before attaching an image or providing a URL.",
			});
		}
	});
