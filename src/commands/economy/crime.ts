...
export const command = new Command("crime", "Try your chances on doing crime!")
	.setExecutor(async ({reply, user}) => {
		if (user.id in cooldowns && cooldowns[user.id] >= Date.now()) {
			await reply(
				format(
					...
					pms(cooldowns[user.id] - Date.now(), {compact: true, secondsDecimalDigits: 1})
				)
			);
			return;
		}
		...
		if (answer === "Failure") {
			...
			await reply(format(sampleArray(text.commands.crime.failure), `\`$${-obtained}\``));
		} else {
			const info = await upsertUserInfo(user);
			const obtained = randRange(...constants.crime.amountRange);
			cooldowns[user.id] = Date.now() + constants.crime.cooldownMs;
			await db.userInfo.update({ where: { id: info.id }, data: { balance: { increment: obtained } } });
			await reply(format(sampleArray(text.commands.crime.sucess), `\`$${obtained}\``));
		}
	});
