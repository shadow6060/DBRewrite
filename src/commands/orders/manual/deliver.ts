...
import { ChannelType, GuildChannel, StringSelectMenuBuilder, CommandInteraction, ComponentType, EmbedBuilder, PartialGroupDMChannel, MessageFlags } from "discord.js";
...
export const command = new ExtendedCommand(
	...
	.setExecutor(async (int: CommandInteraction) => {
		...
		if (orders.length === 0) {
			await int.reply({ content: "No orders available for delivery.",flags: MessageFlags.Ephemeral });
			return;
		}
...
client.on("interactionCreate", async (interaction) => {
	...
	if (componentId === "deliver_order") {
		...
		if (!order) {
			await interaction.reply({ content: "Invalid order selected.", flags: MessageFlags.Ephemeral });
			return;
		}
