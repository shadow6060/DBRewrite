...
export const command = new Command(
	...
	.setExecutor(async int => {
		const commandName = "brew"; // Command name is "brew" in this case

		// Track command usage
		await upsertWorkerInfo(int.user, commandName);
		...
		if (subcommand === "attach") {
			...
			if (!int.channel) {
				throw new IllegalStateError("Channel is not available.");
			}
			const confirmFilter = (i: MessageComponentInteraction<"cached">) => i.customId === "confirm_brew" && i.user.id === int.user.id;
			...
			imageUrl = int.options.get("url", true).value as string;
			...
			const confirmFilter = (i: MessageComponentInteraction<"cached">) => i.customId === "confirm_brew_url" && i.user.id === int.user.id;
			const confirmCollector = int.channel.createMessageComponentCollector({filter: confirmFilter, time: 15000});
